...
already in place? Existing work offers many controller families, but rarely
compares them under shared alpha, timing, cost, and data assumptions, making
it difficult to separate forecasting quality from control quality. This paper
introduces a frozen-bundle evaluation protocol, meaning a pre-committed
artifact in which hyperparameters, splits, and cost assumptions are locked
before controller results are examined, together with a risk-shaping operator
framework that interprets control as acting on outcome distributions through
selection operators, outcome operators, or both; empirically, the protocol
compares 19 controllers on top of the same factor/GARCH/HMM alpha engine and
constrained allocator across two disjoint equity universes. Across both
universes, the robust family defines the frontier: convexity-enhanced CVaR is
strongest when left-tail asymmetry is pronounced, plain CVaR is the more
portable robust baseline, and learning helps mainly as meta-control over
structured experts rather than as the primary control layer.
\end{abstract}
...
control, and RL are all plausible answers to that question. Viewed abstractly,
these controllers can be treated as operators on induced outcome
distributions: they may change which actions are selected, how outcomes are
transformed, or both.
...
\item A risk-shaping operator framework that classifies controllers by how they
      ...
      outcome operators, or mixtures of policies, and that suggests a broader
      view of constrained control as acting on outcome distributions.
...
rather than as the primary decision layer. Portfolio management is the concrete
test bed here, but the underlying claim is broader: admissible controllers can
be understood as operators that reshape outcome distributions under
uncertainty.
...
\cite{gu2020,kolm2019,jander2025}. Recent applied work continues to study how
agent architecture, rebalancing frequency, and signal design change
portfolio-RL behavior in realistic market settings \cite{ayari2025}.
...
allocator.
\subsection{Receding-Horizon Alpha--Allocator--Controller Formulation}

To make the modular stack precise, it is useful to write the
alpha--allocator--controller pipeline as a discrete-time receding-horizon
system. Let $x_t \in \mathcal{X}$ denote the controller state, let
$u_t \in \mathcal{U}$ denote a trade or exposure-control action, and let
$\xi_t \in \Xi$ denote exogenous market disturbances. The state aggregates the
current book, cash, latent risk factors, regime beliefs, and any compact
summaries required by downstream control. The system evolves according to
\[
x_{t+1} = f(x_t, u_t, \xi_t).
\]

The alpha layer maps observations $o_t$ into predictive objects
\[
z_t = \mathcal{A}_{\theta}(o_t) = (\alpha_t, \Sigma_t, s_t),
\]
where $\alpha_t$ is the expected-return vector, $\Sigma_t$ is a risk estimate,
and $s_t$ is a latent or filtered regime state. The allocator then turns those
predictive objects into a reference portfolio
\[
w_t^{\mathrm{ref}} = \mathcal{G}(z_t).
\]
In the present architecture, $w_t^{\mathrm{ref}}$ is the constrained allocator
book anchored to $w_t^{\text{target}}$. Over a planning horizon $H$, the same
mapping can be viewed as generating a reference trajectory
\[
...
Given the current state $x_t$ and the reference trajectory
$\mathbf{w}_t^{\mathrm{ref}}$, the predictive controller solves
\[
\begin{aligned}
\min_{\{u_{t+i|t}\}_{i=0}^{H-1}} \quad &
\sum_{i=0}^{H-1}
\ell(x_{t+i|t}, u_{t+i|t}; w_{t+i|t}^{\mathrm{ref}}, z_{t+i|t}) \\
&+ \ell_f(x_{t+H|t}) \\
\text{s.t.} \quad &
x_{t+i+1|t} = f(x_{t+i|t}, u_{t+i|t}, \xi_{t+i|t}), \\
& x_{t+i|t} \in \mathcal{X}, \quad u_{t+i|t} \in \mathcal{U}, \\
& x_{t|t} = x_t, \quad x_{t+H|t} \in \mathcal{X}_f.
\end{aligned}
\]
...
u_t = u_{t|t}^{\star},
\]
and the optimization is repeated at the next step. This makes the alpha layer
the forecast generator, the allocator the reference generator, and the control
layer the constrained tracking-and-risk-shaping mechanism.

A convenient stage-cost decomposition is
\[
\begin{aligned}
\ell(x,u; w^{\mathrm{ref}}, z)
&=
\ell_{\mathrm{track}}(x; w^{\mathrm{ref}})
+ \ell_{\mathrm{trade}}(u) \\
&\quad
+ \ell_{\mathrm{risk}}(x,u; z)
+ \ell_{\mathrm{reg}}(x,u),
\end{aligned}
\]
...
\ell_{\mathrm{track}}(x; w^{\mathrm{ref}})
=
\lVert h(x) - w^{\mathrm{ref}} \rVert_Q^2,
\]
\[
\ell_{\mathrm{trade}}(u)
=
u^\top R u
\quad \text{or} \quad
\lambda_{\mathrm{to}} \lVert u \rVert_1,
\]
and
\[
\ell_{\mathrm{risk}}(x,u; z)
=
\lambda_{\mathrm{risk}} \rho(x,u; z),
\]
where $h(x)$ extracts portfolio weights from the state and $\rho$ may encode
volatility, CVaR, drawdown, or regime-dependent penalties. For implementation,
a local linearization
\[
x_{t+1} = A_t x_t + B_t u_t + c_t
\]
yields a structured quadratic or convex program whose coefficients depend on
the current forecast bundle and regime state.

In the minimal self-financing tracking case, the state is only the weight
vector $w_t \in \mathbb{R}^N$ and the dynamics reduce to
\[
w_{t+1} = w_t + u_t.
\]
The corresponding MPC problem becomes
\[
\begin{aligned}
\min_{\{u_{t+i|t}\}_{i=0}^{H-1}} \quad &
\sum_{i=0}^{H-1}
\lVert w_{t+i|t} - w_{t+i|t}^{\mathrm{ref}} \rVert_Q^2 \\
&+ \sum_{i=0}^{H-1}
u_{t+i|t}^\top R u_{t+i|t} \\
&+ \lambda \sum_{i=0}^{H-1}
\rho(w_{t+i|t}, u_{t+i|t}) \\
&+ \lVert w_{t+H|t} - w_{t+H|t}^{\mathrm{ref}} \rVert_P^2 \\
\text{s.t.} \quad &
w_{t+i+1|t} = w_{t+i|t} + u_{t+i|t}, \\
& \mathbf{1}^\top w_{t+i|t} = 1, \\
& w_{t+i|t} \in \mathcal{W}, \quad u_{t+i|t} \in \mathcal{U}.
\end{aligned}
\]

When the forecasting module is learned, it simply parameterizes the control
problem through $z_t = \Phi_{\theta}(o_t)$. The executed action can then be
written compactly as
\[
u_t^{\star} = \Psi_H(x_t, \Phi_{\theta}(o_t)),
\]
where $\Phi_{\theta}$ denotes the learned forecasting-and-allocation map and
$\Psi_H$ denotes the horizon-$H$ constrained optimizer. Candidate H in the
frozen-bundle is the most direct instantiation of this receding-horizon view,
while the other controllers can be interpreted as simpler or more heavily
parameterized special cases of the same alpha--allocator--controller stack.

\paragraph{Connection to operator framework.}
The receding-horizon formulation defines the selection operator
$\mathcal{Q}_{\pi}$ through constrained optimization over action sequences.
Specifically, the optimizer $\Psi_H$ induces a mapping from the reference
distribution generated by the alpha--allocator pipeline to a controlled
distribution via sequential action selection. Additional layers, such as
convexity-aware payoff shaping, introduce outcome operators
$\mathcal{T}_{\pi}$ acting on realized returns. Together, these yield the
factorization
\[
F_t^\pi = (\mathcal{T}_{\pi}\circ\mathcal{Q}_{\pi})(F_t^{(0)}),
\]
linking the mechanistic control formulation to the distributional
risk-shaping perspective.

\paragraph{Definition (risk-shaping MPC controller).}
An MPC controller is \emph{risk-shaping} if its receding-horizon objective or
constraints are specified in terms of downside-distribution control rather than
tracking alone. Concretely, given forecasts $z_t$ and reference weights
$w_{t:t+H|t}^{\mathrm{ref}}$, a risk-shaping MPC selects
\[
u_t^\star \in \arg\min_{\mathbf{u}_{t:t+H-1|t}}
\sum_{i=0}^{H-1}
\ell_{\mathrm{track}}
\;+\;
\ell_{\mathrm{trade}}
...
\ell_{\mathrm{reg}}
\]
subject to portfolio dynamics and admissibility constraints, where
$\ell_{\mathrm{risk}}$ or the constraint set explicitly encodes a downside
functional such as volatility, drawdown, stress loss, or CVaR. In operator
language, such a controller implements a structured selection operator
$\mathcal{Q}_{\pi}^{\mathrm{MPC}}$ on the induced return distribution. If it
is further combined with an explicit payoff-shaping layer, it becomes a
composed risk-shaping controller
$\mathcal{S}_{\pi}^{\mathrm{RSMPC}}
= \mathcal{T}_{\pi}\circ\mathcal{Q}_{\pi}^{\mathrm{MPC}}$.
The archived H controller is therefore a \emph{selection-only} risk-shaping
MPC: it plans over future allocations with turnover, stabilizer, and
stress-aware penalties, but it does not by itself add a nontrivial outcome
operator.

A minimal practical extension upgrades this to a discrete
\emph{risk-shaping MPC} by choosing a convexity mode jointly with the control
plan. Writing
\[
T_m(r)=r-c_m+\lambda_m(\tau-r)_+^2,
\qquad m_t \in \{0,1,2\},
\]
the controller may execute
\[
(u_t^\star, m_t^\star)
\in
\arg\min_{u_t,m_t}\;
\mathbb{E}\!\left[
\ell(x_t,u_t;w_t^{\mathrm{ref}},z_t)
+ \rho\!\left(T_{m_t}(Q(u_t))\right)
\right].
\]
This is the smallest implementation that turns a selection-only MPC into a
composed controller with an explicit outcome operator selected inside the
planning loop.
...
\item \textbf{Predictive control.} A risk-shaping model-predictive controller
      (MPC) performs receding-horizon exposure planning with explicit turnover,
      stress, and stabilizer penalties, providing a structured
      control-theory comparator and a concrete selection-operator
...
\paragraph{Operator view of control.}
The representation
\[
F_t^\pi = (\mathcal{T}_\pi \circ \mathcal{Q}_\pi)(F_t^{(0)})
\]
admits a broader interpretation: any admissible controller can be viewed as an
operator acting on a reference outcome distribution. The selection operator
$\mathcal{Q}_\pi$ modifies the distribution through constrained action
selection, while the outcome operator $\mathcal{T}_\pi$ modifies realized
payoffs. This decomposition is not specific to portfolio management. It applies
more broadly to sequential decision systems in which actions induce
distributions over outcomes and control modifies either the selection of
actions or the transformation of outcomes.

\paragraph{Framework Proposition (operator decomposition of control policies).}
Consider a decision system in which a baseline mechanism induces an outcome
distribution $F^{(0)}$, and a controller $\pi$ modifies admissible actions
subject to constraints. Under mild measurability conditions, the induced
outcome distribution can be written as
\[
F^\pi = (\mathcal{T}_\pi \circ \mathcal{Q}_\pi)(F^{(0)}),
\]
where $\mathcal{Q}_\pi$ is a selection operator induced by constrained action
choice and $\mathcal{T}_\pi$ is an outcome transformation operator acting on
realized outcomes. In particular, optimization-based controllers correspond to
structured selection operators, learning-based controllers approximate such
operators from data, and payoff-shaping mechanisms introduce nontrivial
outcome operators.

\paragraph{Framework prediction.}
The operator framework is falsifiable rather than merely taxonomic. It predicts
that nontrivial outcome operators should add value only when the
selection-optimized reference distribution still retains pronounced residual
left-tail mass. In the current study we test that prediction across two
disjoint universes with different released tail profiles. Using the
control-comparison factor-only benchmark as a proxy for the reference book,
Universe A has mean daily 5\% VaR about $-1.19\%$ and mean maximum drawdown
about $-19.5\%$, versus about $-1.15\%$ and $-12.0\%$ in Universe B across the
same train-fraction grid. The framework therefore predicts a larger role for
convexity-aware payoff shaping in Universe A than in Universe B.

The controller search problem is therefore
\[
\begin{aligned}
\pi^\star
...
This operator lens also connects the controller comparison to classical
distortion-risk ideas. Dual-theory and distorted-expectation views interpret
risk control as transforming probabilities or outcomes rather than only
changing point forecasts \cite{yaari1987,schmeidler1989,wang2000}. In that
language, CVaR is a coherent left-tail distortion applied during portfolio
selection, while the convexity layer is a discrete outcome distortion applied
to realized returns. The framework therefore does more than label winners: it
classifies controllers by how they reshape the induced return distribution.

The relationship to the distortion-risk literature is therefore substantive
rather than cosmetic. Yaari's dual theory and Wang-type distortion operators
emphasize that decision quality can be changed by distorting probabilities or
outcomes instead of only changing point forecasts \cite{yaari1987,wang2000}. In
the present finite-horizon control setting, $\mathcal{Q}_{\pi}$ plays the role
of an action-induced distributional distortion generated by constrained
selection, while $\mathcal{T}_{\pi}$ captures explicit outcome reshaping after
selection. The contribution here is to move that logic into a modular control
pipeline in which optimization, learning, and payoff shaping can be compared in
operator form under shared information and cost assumptions.
...
Proposition~1 instantiates this framework by characterizing when a specific
outcome operator $\mathcal{T}_{\pi}$, namely the convexity layer, improves
tail risk relative to the identity operator.
...
\paragraph{Proposition 2 (selection-only limitation under residual tail gap).}
Let $\mathfrak{Q}$ denote the admissible class of selection operators and
assume there exists
\[
Q^\star \in \arg\min_{Q \in \mathfrak{Q}}
\operatorname{CVaR}_\alpha(-Q(F^{(0)})).
\]
Suppose further that the selected distribution $Q^\star(F^{(0)})$ still has
positive mass in a protected tail region where the payoff transformation
$g_m(r)=r-c(m)+\lambda_m(\tau-r)_+^2$ satisfies the conditions of
Proposition~1 with $\delta_\alpha(m)>0$. Then for every selection-only
controller $Q \in \mathfrak{Q}$,
\[
\operatorname{CVaR}_\alpha(-Q(F^{(0)}))
\ge
\operatorname{CVaR}_\alpha(-g_m(Q^\star(F^{(0)}))) + \delta_\alpha(m).
\]
In particular, once the selection-optimal distribution still contains residual
protected-tail mass, no admissible selection-only controller can match the
left-tail improvement delivered by the composed controller
$\mathcal{T}_{\text{convex}}\circ Q^\star$.

\emph{Proof sketch.} By definition of $Q^\star$,
$\operatorname{CVaR}_\alpha(-Q(F^{(0)})) \ge
\operatorname{CVaR}_\alpha(-Q^\star(F^{(0)}))$ for every
$Q \in \mathfrak{Q}$. Proposition~1 gives
$\operatorname{CVaR}_\alpha(-g_m(Q^\star(F^{(0)}))) \le
\operatorname{CVaR}_\alpha(-Q^\star(F^{(0)}))-\delta_\alpha(m)$.
Combining the two inequalities yields the claim. The proposition therefore
formalizes the residual-tail case in which a pure selection operator is
insufficient and an outcome operator is required for additional improvement.
...
paths are listed in the appendix. Throughout the paper, a \emph{frozen-bundle}
means a pre-committed evaluation artifact in which hyperparameters, splits, and
cost assumptions are locked before controller results are examined.
...
The main empirical claims are directly tied to released bundle artifacts.
Table~\ref{tab:metrics} is reproduced from
\texttt{research\_control\_comparison.csv},
Table~\ref{tab:robust_significance} from
\texttt{research\_control\_significance.csv}, and the appendix tables and
figures from the correspondingly named CSV and PNG files listed in Appendix
A.2. Reproducing the paper therefore requires only the public repository, the
two released universe bundles, and the pinned timing, cost, and split settings
stored with each bundle.
...
\paragraph{How to read the bootstrap evidence.}
The block-bootstrap intervals summarize uncertainty in Sharpe differences within
the archived reference split, not certainty about every future market. A
non-decisive interval should therefore be read as ``the current sample does not
separate these leaders cleanly'', not as proof that the methods are equivalent.
This matters most in Universe B, where the leading methods cluster tightly and
the honest interpretation is that a longer sample or additional universes would
be needed to separate them with more statistical confidence.
...
Annualized figures. The unusually strong Universe-A return for plain D reflects
scenario-based reweighting inside the CVaR optimizer rather than a separate
alpha source.}
\label{tab:metrics}
...
method. The cleaner interpretation is that gate complexity is not the
bottleneck: the simpler council beats the MLP gate because both remain
selection-only operators, whereas the leading controller composes selection
with payoff shaping.
...
supported only where left-tail asymmetry is pronounced. Resolving the tighter
Universe-B cluster likely requires either a longer out-of-sample history,
additional zero-overlap universes, or more regime-diverse evaluation windows,
because the present sample does not contain enough separation to distinguish
the leading methods statistically.
...
primary control. The council's edge over the MLP gate shows that gate
complexity is not the limiting factor. The deeper limitation is structural:
both controllers remain selection-only operators, whereas the best-performing
stack combines selection with an explicit outcome operator.
...
\paragraph{MPC as the defensive frontier.}
If the binding objective is drawdown minimization rather than maximal Sharpe,
H\_mpc is the clearest answer in the bundle. In Universe B it reaches a
$-3.9\%$ maximum drawdown at 0.64 Sharpe, the shallowest drawdown profile in
the entire comparison by a wide margin, and in Universe A it still limits
drawdown to $-6.7\%$. The practical takeaway is therefore two-part: D or D+
for the risk-adjusted-return frontier, H\_mpc for the defensive frontier.
...
\paragraph{Generalization beyond finance.}
The operator-based view of control extends beyond portfolio management. In
general sequential decision systems, one may distinguish between mechanisms
that modify action selection under constraints and those that reshape outcome
distributions after selection. This suggests a broader perspective in which
control, planning, and learning methods can be unified through the operators
they induce on outcome distributions.
...
\paragraph{Broader-impact note.}
The study uses public market and macro data and is intended as methodological
evidence rather than investment advice. Backtests remain vulnerable to
survivorship effects, evolving market structure, and the gap between simulated
and realized execution. For that reason, the paper frames its claims as
comparative statements about controller design under shared assumptions, not as
promises of deployable outperformance.
...
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, topsep=4pt]
\item Tabular summaries:
      \texttt{research\_metrics.csv},
      \texttt{research\_control\_comparison.csv},
      \texttt{research\_ablation\_summary.csv}, and
      \texttt{research\_robustness\_summary.csv}.
\item Regime and benchmark diagnostics:
      \texttt{research\_regime\_summary.csv} and
      \texttt{research\_rolling\_references.csv}.
\item Significance and test artifacts:
      \texttt{research\_bootstrap\_cis.csv},
      \texttt{research\_bootstrap\_significance.csv},
      \texttt{research\_control\_significance.csv},
      \texttt{research\_jobson\_korkie.csv}, and
      \texttt{research\_ts\_cv.csv}.
\item Bundle metadata:
      \texttt{research\_summary.json}.
\item Main comparison figures:
      \texttt{control\_method\_overview.png},
      \texttt{control\_pareto\_frontier.png}, and
      \texttt{control\_interpretability.png}.
\item Tail and controller diagnostics:
      \texttt{control\_mpc\_diagnostic.png} and
      \texttt{control\_tail\_diagnostic.png}.
\item Legacy supplementary figures:
      \texttt{pipeline\_rolling\_windows.png} and
      \texttt{pipeline\_reward\_ablation.png}.
\end{enumerate}
...
three train-fraction splits. The takeaway is that Universe A has a clearer
robust hierarchy, whereas Universe B exhibits a tighter cluster among the
leading robust, council, and MPC methods.}
...
by regime. The takeaway is that the learned gates mostly reallocate around a
CVaR-heavy core rather than replacing robust control with a distinct learned
policy.}
...
% ============================================================================
% RECENT RL FOR PORTFOLIO MANAGEMENT
% ============================================================================

\bibitem{ayari2025}
R. Ayari.
\newblock Feature configuration effects in DRL portfolio management: A
  risk-focused evaluation under market stress.
\newblock \emph{Quantitative Finance}, 26(1):119--136, 2026.
