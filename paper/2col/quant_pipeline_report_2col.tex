...
\begin{table*}[t]
\centering
\caption{Legacy ablation summary across train/test split settings for the retained pruning suite.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{../../results/20260330_033844/control_tail_diagnostic.png}
\caption{Left-tail empirical distribution diagnostic for the reference split. Compared with factor-only and plain CVaR, \texttt{D+} shifts the worst-return mass inward and improves the 5\% tail summary while leaving average return roughly unchanged relative to plain \texttt{D}. At the reference split, the D+ layer improves 5\% CVaR by about 30 bps versus plain D, so the improvement is driven primarily by tail-risk reduction rather than by higher average returns alone.}
