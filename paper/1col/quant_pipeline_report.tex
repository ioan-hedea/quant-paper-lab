...
The supported conclusion is therefore not that learning is the preferred
control layer, but that systematic controller comparison is the right research
question and that, in this frozen bundle, portfolio control is best understood
as risk shaping and robust optimization refined by convexity-aware payoff
shaping is the most defensible control mechanism.
\end{abstract}
...
The finished frozen bundle shows that convexity-aware robust optimization now
provides the strongest overall control outcome, that simple regime-aware rules
are already competitive, and that the learning-based methods in this study do
not beat the best optimization-based stack.
The more novel claim is not merely that robust optimization beats RL in noisy
financial data, which practitioners may already suspect. It is that the
controllers in this study can be organized under one risk-shaping framework,
and that the winning architecture combines two separable mechanisms: tail-aware
selection of outcomes through CVaR and tail-aware reshaping of outcomes through
convex payoff transformation.
...
95\% CI $[0.02, 0.77]$, $p=0.045$), even though the return difference is not
yet significant at that split.
...
Q-learning RL. The new artifact bundle also includes a full reference-split
bootstrap table for the headline extension \texttt{D\_plus\_convexity} against
the entire controller set plus SPY.
...
\caption{Control-comparison overview of the March 30 frozen bundle. The upper-left panel ranks all controllers by mean Sharpe, where \texttt{D+} leads, plain \texttt{D} remains the strongest non-extension baseline, and the MLP meta-controller is the strongest learned challenger. The remaining panels summarize return--drawdown trade-offs, the best method in each controller family, and Sharpe stability across train fractions.}
...
explain why meta-control is the strongest learned family in the paper.
...
useful refinement layer rather than a replacement for the robust stack.
...
\caption{Interpretability diagnostics for the revised extensions. Left: the
reference-split MLP meta-controller keeps the largest weight on
\texttt{D\_cvar\_robust} while adaptively reallocating the remainder across
regime rules and LinUCB, which explains why it improves on the standalone
learners while still remaining below \texttt{D+}. Right: convexity mode usage
for \texttt{D\_plus\_convexity} shifts from mostly \texttt{none} in bull states
to mostly \texttt{strong} in bear states. The figure supports the paper's
headline mechanism claim: the gains come from state-dependent distribution
shaping and expert selection, not from a new predictive signal.}
...
tail-risk reduction rather than by higher average returns alone.}
...
established across many independent market environments. The controller
significance analysis is now substantially stronger, but it is still not
exhaustive in the strongest possible sense. The new artifact bundle includes
5{,}000-sample reference-split block-bootstrap tables for both plain
\texttt{D\_cvar\_robust} and \texttt{D\_plus\_convexity} versus the full set of
controller baselines plus SPY. Those tests now show direct Sharpe separation
for D+ versus factor-only, regime rules, LinUCB, \texttt{G\_mlp\_meta}, and
SPY, but not versus plain \texttt{D}. What remains future work is an
aggregated multi-split or multi-window significance analysis, together with
equally complete tables for the broader meta-controller family.
...
more systematic benchmark tuning, and deeper analysis of when council-style
meta-control or convexity modes are activated. If more advanced learned
...
separation versus plain CVaR-robust. Equally
importantly, the paper leaves behind a frozen-bundle evaluation protocol that
can support future controller comparisons under a consistent empirical
standard. In the reported sample, the evidence supports a stronger conceptual
claim than the earlier draft: portfolio control is best understood here as
return-distribution shaping. Explicitly distribution-aware optimization refined
by convexity-aware payoff shaping is the strongest current control layer, and
additional learning complexity is not necessary for daily portfolio
rebalancing on top of the same finance-first alpha engine.
...
writes the artifacts below into \texttt{results/20260330\_012342/}:
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, topsep=4pt]
\item \texttt{research\_metrics.csv}: tabular output for ablation and robustness runs;
\item \texttt{research\_control\_comparison.csv}: split-aggregated controller comparison across the 17 evaluated methods;
...
\item \texttt{research\_control\_significance.csv}: controller-to-controller bootstrap deltas centered on the robust baseline;
\item \texttt{research\_control\_significance\_full.csv}: 5{,}000-sample reference-split bootstrap deltas for plain \texttt{D} and \texttt{D+} versus the full controller set plus SPY;
\item \texttt{research\_control\_significance\_dplus.csv}: the D+-centered slice of that full significance table;
...
intentionally focuses on figures produced directly by the revised timestamped
