...
and a stylized but nontrivial transaction-cost model. On the current backtest
window, the full pipeline attains equity-like return with a materially improved
drawdown profile relative to SPY while remaining close to the strongest
factor-only benchmark in upside capture.
\end{abstract}
...
system works.'' It is the following: \emph{a factor-anchored control layer can
convert a strong alpha engine into a lower-drawdown strategy and can improve
risk-adjusted performance relative to passive equity exposure under explicit
cost and timing assumptions, even when it does not dominate the raw alpha
benchmark on every metric.} The rest of the paper is organized to test that
...
\item multiple train/test split fractions to test dependence on a single
      partition choice;
\item rolling-window walk-forward evaluations to assess subperiod stability;
\item sensitivity sweeps over transaction costs, rebalance bands, hedge ladders,
      and macro lag assumptions; and
\item a strict timing-discipline run that disables the static SEC prior and
...
This section is organized around four empirical questions rather than around the
diagnostic plots themselves. First, does the full pipeline produce a strong
headline result on the main walk-forward window? Second, what does each module
contribute in ablation? Third, do the improvements survive across rolling
subperiods rather than only on one realized path? Fourth, does the controller
behave differently across regimes in a way that matches the architecture's
intended role?
...
fractions. In the current run, the median full-pipeline Sharpe across rolling
windows is 1.65, the median Calmar is 3.26, and the median maximum drawdown is
$-6.8\%$. The full pipeline beats SPY on both Sharpe and Calmar in 100\% of the
available rolling windows, and it beats the factor benchmark on drawdown in
100\% of windows. At the same time, it does \emph{not} beat the factor
benchmark on Calmar in the current rolling sample. That nuance matters: the
controller appears reliably valuable for downside reshaping, but not yet
universally superior to the raw factor engine on all risk-adjusted metrics.
...
features, and richer hedging instruments.
...
The current robustness summaries are also based on only two rolling windows,
which is enough to move beyond a single-path narrative but not enough to claim
stable distributional estimates over many market environments. The workshop
interpretation should therefore be that the robustness engine is now part of the
methodology, not that robustness has been exhaustively established.
...
options or implied-volatility information. The evaluation engine can also be
extended to block bootstrap resamples, additional universes, and deeper
cross-validation across disjoint market regimes.
