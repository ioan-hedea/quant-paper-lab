...
bundle spanning March 2021 to March 2026. In that archived evaluation, the full
pipeline improves maximum drawdown by 8.6 percentage points relative to SPY
($-10.2\%$ versus $-18.8\%$) and exceeds an end-to-end PPO baseline by 8.1
percentage points of annualized return (17.2\% versus 9.1\%). However, the
factor-only benchmark and simple rule-based overlays still achieve materially
higher Sharpe ratios, around 1.9 versus 1.23 for the full RL stack. The
supported conclusion is therefore deliberately narrow: RL adds value here
primarily through interpretable downside control rather than through top-line
...
explicit cost and timing assumptions, can improve downside control relative to
passive exposure while remaining more interpretable than an end-to-end policy.}
\emph{The paper does not claim that this RL layer surpasses the strongest
finance-first or rule-based baselines on return- and Sharpe-based comparisons.}
...
In the archived experiment reported here, it chooses one of four hedge
intensities. The total one-period return can be
...
The reported hedge sleeve should therefore be interpreted as a tractable
convex-loss proxy, not as a market-consistent options strategy.
...
The archived bundle used in the paper contains approximately 1{,}250 daily
observations from late March 2021 through late March 2026. The reported main
split trains the upstream statistical components on the initial segment through
March 23, 2023 and then evaluates the walk-forward controller from March 23,
2023 to March 24, 2026. The manuscript is tied to this frozen archived bundle,
used as the latest complete saved research run for the current configuration,
even though the live codebase may already contain extensions not exercised by
the reported evidence. This does not eliminate prior exploratory iteration, but
it does make the reported evidence bundle explicit and reproducible.
Within the archived suite itself, action ladders, optimizer settings, reward
choices, and comparison baselines are then held fixed across ablations,
split-fraction tests, and rolling-window robustness slices.
...
addition to SPY buy-and-hold, equal-weight, and the factor-only benchmark, the research
...
diversification benefits. The factor-only benchmark isolates the pure alpha layer without any
RL overlay. Comparing the full pipeline against the factor-only benchmark is particularly
...
whether a threshold rule can replicate the hedge sleeve's protection. The
end-to-end PPO
...
against rule-based overlays and end-to-end PPO, rolling robustness slices, and
regime-conditioned behavior. The organizing question throughout is whether the
data support the narrower claim that RL helps more as a controller than as a
predictor.

\subsection{Main Result}

The archived research run used for the paper's main evidence uses a test window
from March 23, 2023 to March 24, 2026. The summary metrics are shown in
Table~\ref{tab:metrics}. This is still only one realized path, but it is the
same saved checkpoint that also feeds the ablation, robustness, and significance
artifacts discussed in the remainder of the paper.

\begin{table*}[t]
\centering
\small
\caption{Performance summary on the walk-forward test window
  (23 Mar 2023 -- 24 Mar 2026). Annualized figures. Max DD denotes maximum
...
\midrule
Full pipeline            & 17.2\% & 11.1\% & 1.23 & $-$10.2\% & 1.68 \\
SPY                      & 17.9\% & 15.7\% & 0.92 & $-$18.8\% & 0.96 \\
Equal-weight            & 22.8\% & 11.7\% & 1.65 & $-$14.8\% & 1.54 \\
Factor-only benchmark    & 33.1\% & 15.3\% & 1.94 & $-$16.5\% & 2.01 \\
Vol-target               & 28.9\% & 13.2\% & 1.92 & $-$14.8\% & 1.95 \\
DD-Delever               & 30.2\% & 13.7\% & 1.95 & $-$14.9\% & 2.02 \\
E2E RL (PPO)             & 9.1\% & 9.1\% & 0.62 & $-$14.0\% & 0.65 \\
\bottomrule
\end{tabular}
\end{table*}

The main result is partly negative and partly favorable, which is exactly what
makes it more credible than the earlier best-case narrative.

Relative to SPY, the full pipeline is almost flat on annualized return, 17.2\%
versus 17.9\%, while materially improving volatility, Sharpe, and drawdown. The
passive-benchmark comparison therefore supports the claim that the controller
can reshape downside risk in a useful way.
The relatively strong equal-weight result is also informative: some of the
benefit in this universe comes from broad diversification across growth,
defensive, and diversifier sleeves rather than from RL control alone.

Relative to the factor-only benchmark, the conclusion is less favorable. The
full pipeline is far below the factor core on both return and Sharpe, so the
reported evidence does not support any stronger claim that RL preserves most of
the upside of the alpha engine.

Relative to the strongest rule-based overlays, the result is also negative. Both
volatility targeting and drawdown-based deleveraging outperform the full
pipeline on return- and Sharpe-based comparisons in the archived sample.

Relative to the end-to-end PPO baseline, the result is directionally favorable.
The modular pipeline substantially outperforms PPO on archived-sample point
estimates and avoids the much
weaker return profile of the monolithic policy.

\noindent\textit{Implication for RQ1 and RQ2.} The main table alone already
suggests a qualified positive-negative result: favorable for modular RL
relative to PPO and passive exposure, but not favorable for RL relative to the
strongest non-RL overlays.

\IfFileExists{results/20260327_experiments/stock_trading/pipeline_research_eval.png}{
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{results/20260327_experiments/stock_trading/pipeline_research_eval.png}
...
rolling-window robustness, and regime-conditional controller behavior. The
...
not from the full RL stack, while also showing that the modular RL stack
remains visibly above the end-to-end PPO baseline.}
\label{fig:research}
\end{figure*}
}{
\IfFileExists{stock_trading/results/20260327_experiments/stock_trading/pipeline_research_eval.png}{
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{stock_trading/results/20260327_experiments/stock_trading/pipeline_research_eval.png}
\caption{Compact workshop-style summary figure generated by the robustness
engine. The four panels show the main equity-curve result, ablation evidence,
rolling-window robustness, and regime-conditional controller behavior. The
figure makes clear that the strongest raw-performance outcomes in the archived
...
plus portfolio RL, alpha plus hedge RL, and the full pipeline across multiple
split settings.
...
\midrule
Factor-only benchmark & 21.7\% & 12.2\% & 1.50 & $-$14.0\% & 1.55 \\
...
Full -- Factor-only benchmark   & -0.70 & [-1.34, -0.26] & 0.000 \\
...
The generated ablation table sharpens the story considerably. Averaged across
split settings, the factor-only benchmark remains the strongest pure alpha expression, with
mean return of 21.7\% and mean Sharpe of 1.50. The pairs and LSTM sleeves add
very little incremental value on top of that base. This is a useful pruning
result: on top of the factor core in the current archived evaluation bundle,
the pairs and LSTM sleeves do not add enough incremental value to justify their
complexity. The full pipeline is
materially weaker on raw upside, at 15.7\%
mean return and 1.10 mean Sharpe, but it posts the best mean drawdown profile
in the table at $-11.3\%$ and the lowest mean volatility at 11.1\%. The
hedge-only ablation is the strongest intermediate RL configuration, with mean
Sharpe 1.18 and mean drawdown $-13.1\%$, while portfolio RL without the hedge
sleeve is the weakest controller variant, at mean Sharpe 0.95. This pattern is
informative: exposure control on its own can under-express the alpha engine, and
most of the RL contribution appears to come through protection and joint
portfolio-plus-hedge coordination rather than through aggressive risk taking.

\noindent\textit{Implication for RQ3.} Supported. The ablation evidence points
much more strongly to the protection sleeve, and to the joint
portfolio-and-hedge design, than to portfolio RL alone.
...
strong comparators: both deliver around 29--30\% annualized return with Sharpe
ratios near 1.93--1.95, well above the full pipeline's 1.23. The bootstrap
significance table makes the same point more formally. Relative to vol-target
and drawdown-based deleveraging, the full pipeline's Sharpe deficit is around
0.69 to 0.72, and the 95\% confidence intervals remain below zero. The
significance evidence is therefore asymmetric: the full pipeline's Sharpe
deficits relative to the factor-only benchmark and the strongest rule-based
overlays survive bootstrap uncertainty, while its apparent advantage over SPY
and PPO does not reach comparable statistical strength in this archived sample.
...
rule-based overlays on Sharpe or return. RQ1 receives stronger directional
support. The end-to-end PPO baseline reaches only 9.1\% annualized return and
0.62 Sharpe, so the modular architecture substantially outperforms PPO on point
estimates, even though the bootstrap-based Sharpe delta is not statistically
decisive at conventional levels. Here point estimates refer to the archived
sample's observed annualized return, Sharpe, and drawdown metrics before
bootstrap uncertainty qualification. That is still an important result: the data
support RL as a control layer much more strongly than RL as a monolithic
trading policy.

\noindent\textit{Implication for RQ1.} Supported directionally: modular RL is
materially better than end-to-end PPO in point estimates, but the manuscript
does not claim a statistically decisive Sharpe advantage.

\noindent\textit{Implication for RQ2.} Not supported in the archived sample: the
strongest simple non-RL overlays outperform the RL controller on return and
Sharpe.

\subsection{Reward-Function Sensitivity}

Reward design affects where the controller lands on the risk-return frontier,
but it does not overturn the broader architectural story. Across the full
pipeline reward-function ablation, the raw-return reward variant delivers the
highest annualized return at 17.7\% and the highest Sharpe at 1.26, while the
Sortino-style reward produces the best drawdown and Calmar profile, with
maximum drawdown improving to $-9.4\%$ and Calmar rising to 1.78. The default
differential-Sharpe reward is therefore not uniquely optimal; it is simply one
reasonable compromise. This is a useful methodological reminder that reward
choice materially shifts the policy's operating point even when the state and
action spaces are held fixed.
...
\midrule
Differential Sharpe & 16.4\% & 1.18 & $-$11.5\% & 1.42 \\
Return              & 17.7\% & 1.26 & $-$10.2\% & 1.74 \\
Sortino             & 16.8\% & 1.23 & $-$9.4\%  & 1.78 \\
Mean-Variance       & 16.5\% & 1.18 & $-$9.5\%  & 1.73 \\
\bottomrule
\end{tabular}
\end{table}
...
controller's operating point on the risk-return frontier without overturning the
...
fractions. In the archived run, the median full-pipeline Sharpe across rolling
windows is 1.32, the median Calmar is 2.55, and the median maximum drawdown is
$-7.2\%$. The full pipeline beats SPY on Sharpe in 83\% of windows and on
Calmar in 100\% of windows. Relative to the factor-only benchmark, however, the
picture is mixed: the full pipeline posts a better drawdown in 67\% of windows
but a better Calmar in only 17\% of them. That nuance is exactly why the
rolling-window engine matters. The controller appears structurally valuable for
downside reshaping relative to passive exposure, but not yet structurally
dominant over the strongest finance-first benchmark.
These windows still come from one historical sample and should therefore be
interpreted as internal robustness slices rather than independent replications.

The same evidence also helps calibrate the paper's statistical tone. The
bootstrap significance summary shows that the full pipeline's Sharpe advantage
over SPY is positive in point estimate but not statistically decisive, while its
Sharpe disadvantage relative to the factor-only benchmark and the two rule-based overlays is
large enough to survive block-bootstrap uncertainty bands. The appropriate
claim, then, is not that robustness is ``won,'' but that the architecture now
supports a falsifiable and transparent robustness protocol.

\noindent\textit{Implication for the overall claim.} The rolling evidence is
favorable for downside reshaping relative to SPY, but only mixed relative to
the factor-only benchmark and the strongest rule-based alternatives.

\IfFileExists{results/20260327_experiments/stock_trading/pipeline_rolling_windows.png}{
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{results/20260327_experiments/stock_trading/pipeline_rolling_windows.png}
\caption{Rolling-window robustness from the archived March 2026 experiment
bundle. The full pipeline is consistently stronger than SPY on Calmar and
usually stronger on Sharpe, but it remains below the factor-only benchmark and the best
rule-based overlays on median upside-oriented metrics. These rolling windows are
internal robustness slices from one archived historical sample rather than
independent replications.}
\label{fig:rollingwindows}
\end{figure*}
}{
\IfFileExists{stock_trading/results/20260327_experiments/stock_trading/pipeline_rolling_windows.png}{
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{stock_trading/results/20260327_experiments/stock_trading/pipeline_rolling_windows.png}
\caption{Rolling-window robustness from the archived March 2026 experiment
bundle. The full pipeline is consistently stronger than SPY on Calmar and
usually stronger on Sharpe, but it remains below the factor-only benchmark and the best
rule-based overlays on median upside-oriented metrics. These rolling windows are
...
decision system. Figure~\ref{fig:research} provides the main summary used in the
paper, while supplementary alpha and execution diagnostics are reported in
Appendix Figures~\ref{fig:alpha} and \ref{fig:execution}. The regime
summaries show that the controller is not degenerate:
it carries nonzero hedge ratios, nontrivial cash weights, and changing turnover
profiles rather than collapsing to an always-in, never-hedged policy. At the
same time, the archived baseline split shows only modest regime separation. In
the representative full-pipeline split, average portfolio action declines from
roughly 2.08 in bull states to 1.26 in bear states, and average cash weight
rises from about 6.6\% to 10.6\%, but hedge intensity is comparatively flat.
That is a constructive research finding rather than a failure. The controller is
reacting, but mostly through exposure throttling rather than through a strongly
state-contingent hedge sleeve. This behavioral pattern is consistent with the
ablation evidence that the protection sleeve matters, but that the present hedge
policy may still be under-expressive relative to its intended role. This is
exactly the kind of behavioral gap that motivates the paper's forward-looking
interest in implied-volatility-conditioned and option-style hedging overlays.

\noindent\textit{Implication for RQ3.} Supported, but only partially. The
behavioral evidence is consistent with hedge-related control mattering more than
portfolio RL alone, while also suggesting that the hedge policy remains less
state-contingent than intended.

\section{Discussion}

The experimental results support three conclusions of particular methodological importance.

First, the data and alpha side of the pipeline is doing real work. The
factor-only benchmark is strong, and equal-weight also performs reasonably well,
which suggests that the diversified universe and the factor core contribute most
of the signal, while the non-factor sleeves currently add limited incremental
value in the archived evaluation. The relatively strong equal-weight result is
itself informative: part of the observed performance comes from broadening the
universe and diversifying across sectors and defensive sleeves, not only from
the more elaborate control machinery. In the current archived evaluation, the
paper's strongest alpha evidence comes from the factor core, not from the
auxiliary pairs or LSTM sleeves.

Second, the factor-anchored RL design is helpful in a narrower sense than simple
benchmark outperformance. The ablation, rolling-window, and significance results
all suggest that the full pipeline's main contribution is downside-risk
transformation rather than universal metric dominance. The factor-only benchmark
remains the strongest pure alpha engine, and the simple vol-target and
drawdown-based deleveraging overlays are stronger than the RL controller on this
sample's primary return- and Sharpe-based metrics.
...
rule-based overlays on Sharpe or return in the archived sample. Portfolio RL on
its own underperformed and could under-express the alpha engine. The pairs and
LSTM sleeves added little incremental value beyond the factor core. Finally, the
hedge response appears flatter across regimes than the intended design would
prefer. These are useful negative results because they narrow where future
...
shows that the factor-only benchmark and cheap rule-based overlays still dominate
the full pipeline on upside-oriented metrics. Future gains will likely come
from more realistic point-in-time fundamentals, true macro vintage data,
cross-asset regime features, explicit uncertainty conditioning, stronger
statistical validation, and richer hedging instruments with implied-volatility
information.

\subsection{When Practitioners Should Use This Architecture}

The archived evidence suggests a fairly specific use case for factor-anchored RL
control rather than a universal recipe. The architecture is most defensible when
downside protection matters more than raw return maximization, when
interpretability matters for institutional adoption or model governance, when a
credible finance-first alpha engine already exists, and when the research or
production setting can support daily rebalancing together with periodic RL
training and monitoring.

Simpler alternatives are often sufficient in other settings. If the primary goal
is pure Sharpe maximization on this kind of daily equity universe, the archived
bundle suggests that the factor-only benchmark and simple overlays such as
volatility targeting or drawdown-based deleveraging are harder to beat. If
...
when RL is appropriate in finance, it appears more defensible as a controller
over a strong signal engine than as a replacement for prediction itself.
...
archived experiment approximates convex protection without live options data,
and the robustness engine still depends on one underlying historical sample even
when it slices that sample in
multiple ways.
...
still influences comparative conclusions.
Because the evaluation suite reports many metrics and comparator strategies,
some apparent wins and losses should also be interpreted with the usual caution
around multiple comparisons.
...
windows drawn from one historical sample. The workshop interpretation should
therefore be that the robustness engine is now part of the methodology, not
that robustness has been exhaustively established across many independent market
environments.
...
implied-volatility-conditioned option overlays such as protective puts, collars,
and put spreads. A second important line of follow-up work is empirical pruning:
the LSTM sleeve and the pairs sleeve should remain in the architecture only if
ablation continues to show incremental value after the expanded robustness pass.
Finally, while the evaluation engine now includes bootstrap significance
outputs, future work could push further toward formal Sharpe-ratio comparison
tests, additional universes, and deeper cross-validation across disjoint market
regimes.

\section{Conclusion}

This paper shows that a financially grounded hybrid architecture can be modular,
interpretable, and empirically informative even when its RL layer does not beat
the strongest finance-first baselines on top-line performance. Classical quantitative models remain
responsible for alpha discovery, while reinforcement learning acts as a
sequential risk and allocation controller. The resulting system is not a proof
that RL dominates traditional quant methods. Instead, it is a structured
research pipeline whose behavior is interpretable, testable, and falsifiable.
The main empirical takeaway is accordingly narrow but meaningful: when RL is
anchored to a strong finance-first alpha book and evaluated under explicit
timing and cost assumptions, it can improve downside control relative to passive
exposure and outperform end-to-end RL on point estimates, even if strong
factor-first and rule-based baselines still set a higher bar on pure Sharpe and
return. Equally importantly, the paper leaves behind a frozen-bundle evaluation
protocol that can support future pruning, hedge redesign, and options-based
extensions under a consistent empirical standard. In the reported sample, the
evidence supports RL as a disciplined control overlay for downside shaping and
rejects the stronger claim that RL already surpasses the best finance-first or
rule-based alternatives on return and Sharpe.
...
The March 2026 archived experiment bundle used in the main paper results relies
on the discrete action ladders below. The live codebase now contains early
scaffolding for implied-volatility-conditioned, option-style hedge selection,
but those extensions are not yet part of the reported archived evaluation.
...
\path{results/20260327_experiments/stock_trading/} alongside the standard
pipeline figures:
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, topsep=4pt]
...
and Figures~\ref{fig:research} and \ref{fig:rollingwindows}. The appendix also
intentionally omits the older RL dashboard with embedded performance text,
because those annotations reflect an exploratory live run rather than the frozen
archived bundle analyzed in the paper.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{pipeline_alpha_models.png}
\caption{Supplementary alpha diagnostics for the archived bundle. The figure is
diagnostic rather than primary evidence and is consistent with the paper's
pruning conclusion that the factor core carries most of the demonstrable alpha
value in the current evaluation.}
\label{fig:alpha}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.82\textwidth]{pipeline_execution_rl_paper.png}
\caption{Supplementary execution diagnostic for the archived execution
submodule. The figure illustrates turnover management, no-trade band
activations, and stylized transaction-cost behavior, but it is not treated as a
primary claim about live trading execution realism.}
\label{fig:execution}
\end{figure*}
