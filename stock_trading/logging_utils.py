...
DEFAULT_LOG_DIR = Path("stock_trading/logs")
...
def default_log_path(log_prefix: str) -> str:
    """Return a timestamped default log path for stock-trading scripts."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return str(DEFAULT_LOG_DIR / f"{log_prefix}_{timestamp}.log")


def latest_log_alias(log_prefix: str) -> str:
    """Return the stable alias path for the latest stock-trading script log."""
    return str(DEFAULT_LOG_DIR / f"latest_{log_prefix}.log")


@contextmanager
def tee_output_from_env(log_prefix: str | None = None):
    """Enable tee logging via QUANT_LOG_FILE or a default stock-trading log file."""
